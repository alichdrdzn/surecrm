...
const ImportCSV = ({ open, handleClose, fetchdata }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      setProgress(30);
      const result = await apipost('contact/import-csv', formData);
      setProgress(100);
    ...
    >
      <DialogTitle>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Typography variant="h6">{t('Import Contacts from CSV')}</Typography>
      ...
      <DialogContent>
        <Typography variant="body2" sx={{ mb: 2, color: 'text.secondary' }}>
          {t('Upload a CSV file to import contacts. The first row should contain column headers.')}
        </Typography>
        ...
        <Box sx={{ mb: 2 }}>
          ...
          <Typography variant="caption" component="div" sx={{ color: 'text.secondary', lineHeight: 1.6 }}>
            {t('firstName, lastName, dateOfBirth, gender, phoneNumber, emailAddress, address, alternatePhoneNumber, additionalEmailAddress, instagramProfile, twitterProfile, preferredContactMethod, referralSource, referralContactName, relationshipToReferrer, preferredLanguage')}
          </Typography>
        </Box>
